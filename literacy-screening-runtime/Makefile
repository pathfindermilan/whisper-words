...
#################################################################################
# Commands for testing and debugging your submission                            #
#################################################################################
...
## Creates a submission/submission.zip file from the source code in examples
pack-example:
...
ifeq (,$(wildcard ./submission/submission.zip))
	$(error To test your submission, you must first put a "submission.zip" file in the "submission" folder. \
	  If you want to use an example, you can run `make pack-example` first)
endif
	docker run \
		${PLATFORM_ARGS} \
		${TTY_ARGS} \
